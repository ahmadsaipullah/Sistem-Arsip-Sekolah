                                    ...
                                    @forelse ($pegawai as $item)
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <a href="{{ route('data-kepegawaian.show', $item->id) }}"
                                                    class="btn btn-xs btn-info mb-1">
                                                    <i class="fa fa-eye"></i>
                                                </a>
                                                {{-- Tombol Lihat Dokumen --}}

                                                <a href="{{ Storage::url($item->file_dokumen_pdf) }}" target="_blank"
                                                    class="btn btn-xs btn-primary mb-1">
                                                    <i class="fa fa-file-pdf"></i> Lihat
                                                </a>

                                                {{-- Tombol Download --}}
                                                <a href="{{ Storage::url($item->file_dokumen_pdf) }}" download
                                                    class="btn btn-xs btn-success mb-1">
                                                    <i class="fa fa-download"></i> Download Dokumen
                                                </a>
                                                <a href="#"
                                                    onclick="printPDF('{{ asset('storage/' . $item->file_dokumen_pdf) }}')"
                                                ...
                                                @if (auth()->user()->level_id == 2)
                                                    <a href="{{ route('data-kepegawaian.edit', $item->id) }}"
                                                        class="btn btn-xs btn-warning mb-1">
                                                        <i class="fa fa-pen"></i>
                                                    </a>
                                                    <form action="{{ route('data-kepegawaian.destroy', $item->id) }}"
